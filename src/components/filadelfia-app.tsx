    ...
    <main className="min-h-screen bg-background pb-10">
      ...
      <form onSubmit={submit} className="mx-auto max-w-md space-y-5 px-5 pt-6">
        ...
        {kind && <div className="animate-gentle-rise space-y-5">
          ...
          <div className="rounded-2xl border border-dashed border-primary/35 bg-secondary/40 p-4"><div className="flex items-start gap-3"><Camera className="mt-0.5 text-primary" /><div><Label htmlFor="profile-photo" className="text-sm font-bold">{kind === "member" ? "Foto de rosto" : "Foto para a igreja conhecer você (opcional)"}</Label><p className="mt-1 text-xs leading-5 text-muted-foreground">Você pode abrir a câmera ou escolher na galeria.</p></div></div><Input id="profile-photo" type="file" accept="image/*" capture="user" required={kind === "member"} className="mt-4 h-auto rounded-xl bg-card py-3" onChange={(e) => setPhoto(e.target.files?.[0] ?? null)} />{photo && <label className="mt-4 flex items-start gap-3 text-xs leading-5"><Checkbox checked={photoConsent} onCheckedChange={(v) => setPhotoConsent(v === true)} /><span>Autorizo o uso desta foto no diretório de aniversariantes e para identificação pastoral.</span></label>}</div>
...
export function FiladelfiaApp() {
  ...
  if (detail) return <main className="min-h-screen bg-background pb-10"><PageHeader title={detail === "birthdays" ? "Aniversariantes" : detail === "gallery" ? "Aconteceu e Foi Bom" : detail === "prayer" ? "Pedido de Oração" : detail === "social" ? "Talentos do Reino" : detail === "location" ? "Como chegar" : "Central Administrativa"} subtitle={detail === "gallery" ? "Memórias que testemunham a graça de Deus" : undefined} onBack={() => setDetail(null)} />{detail === "birthdays" && <BirthdayView items={birthdays} signedUrls={signedUrls} />}{detail === "gallery" && <GalleryView profile={profile} session={session} items={gallery} signedUrls={signedUrls} reload={reload} />}{detail === "prayer" && <PrayerView session={session} />}{detail === "social" && <div className="mx-auto max-w-3xl px-5 py-6"><div className="rounded-3xl bg-card p-6 shadow-pastoral"><div className="mb-4 grid size-14 place-items-center rounded-2xl bg-secondary text-2xl">🧵</div><h2 className="font-display text-2xl">Curso de corte e costura</h2><p className="mt-3 leading-7 text-muted-foreground">Para maiores informações sobre o curso, compareça à igreja na <strong className="text-foreground">terça-feira, às 17h</strong>, e procure a irmã Patrícia ou o irmão Eduardo.</p></div></div>}{detail === "location" && <div className="mx-auto max-w-3xl px-5 py-6"><div className="overflow-hidden rounded-3xl border bg-card"><iframe title="Mapa da Igreja Presbiteriana Filadélfia" className="h-64 w-full border-0" src="https://maps.google.com/maps?q=Rua+Vicente+de+Lima+Cleto,+250,+Nova+Cidade,+São+Gonçalo,+RJ&output=embed&z=15" loading="lazy" /><div className="p-5"><p className="font-bold">Rua Vicente de Lima Cleto, 250</p><p className="text-sm text-muted-foreground">Nova Cidade · São Gonçalo, RJ</p><Button asChild className="mt-4"><a href="https://www.google.com/maps/search/?api=1&query=Rua+Vicente+de+Lima+Cleto+250+São+Gonçalo+RJ" target="_blank" rel="noreferrer">Abrir no mapa</a></Button></div></div></div>}{detail === "admin" && isAdmin && <AdminView profiles={profiles} gallery={gallery} societies={societies} session={session} reload={reload} />}</main>;
...
function HomeView({ nextCult, content, setDetail, setTab }: { nextCult: { day: string; time: string; title: string }; content: ChurchContent[]; setDetail: (d: Detail) => void; setTab: (t: Tab) => void }) { const actions = [{ label: "Pedido de Oração", icon: HeartHandshake, go: () => setDetail("prayer") }, { label: "Aniversariantes", icon: Gift, go: () => setDetail("birthdays") }, { label: "Aconteceu e Foi Bom", icon: Images, go: () => setDetail("gallery") }, { label: "Ação Social", icon: Sparkles, go: () => setDetail("social") }, { label: "Localização", icon: MapPin, go: () => setDetail("location") }, { label: "Sociedades", icon: Users, go: () => setTab("community") }]; return <div className="px-5 py-6"><section className="rounded-3xl bg-forest-deep p-5 text-primary-foreground shadow-pastoral"><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary-foreground/55">Próximo culto</p><div className="mt-4 flex items-end justify-between gap-4"><div><h2 className="font-display text-2xl">{nextCult.title}</h2><p className="mt-1 text-sm text-primary-foreground/70">{nextCult.day} · {nextCult.time}</p></div><Button variant="hero" size="sm" onClick={() => setTab("cultos")}>Ver cultos</Button></div></section><h2 className="mb-3 mt-7 text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Acesso rápido</h2><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{actions.map(({ label, icon: Icon, go }) => <Button key={label} variant="outline" onClick={go} className="h-auto min-h-28 flex-col items-start whitespace-normal rounded-2xl bg-card p-4 text-left shadow-sm"><span className="grid size-10 place-items-center rounded-xl bg-secondary text-primary"><Icon /></span><span className="mt-2 w-full font-bold leading-5">{label}</span></Button>)}</div><section className="mt-7"><div className="flex items-center justify-between"><h2 className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Avisos recentes</h2><Bell className="size-4 text-muted-foreground" /></div><div className="mt-3 space-y-3">{content.filter((c) => c.content_type === "notice").slice(0, 3).map((c) => <article key={c.id} className="rounded-2xl border bg-card p-4"><h3 className="font-bold">{c.title}</h3><p className="mt-1 line-clamp-3 text-sm leading-6 text-muted-foreground">{c.body || c.summary}</p></article>)}{content.filter((c) => c.content_type === "notice").length === 0 && <Message text="Nenhum aviso publicado no momento." />}</div></section></div>; }
